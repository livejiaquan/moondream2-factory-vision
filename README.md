...
- [使用方式](#使用方式)
  ...
  - [影片安全分析（run_test.py）](#影片安全分析run_testpy)
  - [YOLO Gap 評估框架（evaluation/）](#yolo-gap-評估框架evaluation)
- [系統整合指南](#系統整合指南)
- [硬體需求](#硬體需求)
- [檔案說明](#檔案說明)
...
pip install transformers==4.46.3 torch torchvision Pillow einops opencv-python-headless
```
...
對資料夾內的 `.mkv` / `.mp4` 影片做自動抽幀分析，一次涵蓋 caption、query、detect 全部功能，輸出帶標注圖與統計 CSV。
...
# 自訂抽幀頻率與幀數上限
python run_test.py --data data/ --every 10 --limit 20
```
...
| 類型 | 項目 |
|------|------|
| detect | `person`、`helmet`、`wheel chock` |
| query | 人員是否出現、輪檔是否存在、安全帽缺失、面罩穿戴、異常行為 |
| caption | 每幀自然語言描述 |

輸出結構（位於 `outputs/<run_id>/`）：

```
outputs/20260404T214937Z/
├── results.jsonl      每幀完整推理結果
├── summary.csv        扁平化統計表（可直接用 Excel 開）
├── run_meta.json      執行參數與模型資訊
└── annotated/         標注圖（BBox + 查詢結果）
```

---

### YOLO Gap 評估框架（evaluation/）

針對 YOLO 做不到的語意判斷場景（抽菸、異常行為、遮擋情境等），系統性評估 Moondream 的可行性與誤報率。

```bash
cd evaluation

# 1. 負樣本基線測試（量 False Positive 率）
...
| 場景 | 說明 |
|------|------|
| `smoking_visible` | 抽菸（香菸太小，YOLO 難標註）|
| `abnormal_behavior` | 異常行為（YOLO 只框物件，不理解行為）|
| `person_falling` | 倒地 |
| `missing_helmet` | 安全帽缺失語意判斷 |
| `missing_face_mask` | 面罩缺失語意判斷 |
| `wheel_chock_present` | 輪檔存在確認（正面基準）|
...
| 路徑 | 說明 |
|------|------|
| `moondream.py` | 互動 CLI，支援 caption / query / detect / point |
| `batch.py` | 批量處理整個圖片資料夾 |
| `run_test.py` | 影片抽幀全流程安全分析，輸出 JSONL + CSV + 標注圖 |
| `advanced/moondream3_h100.py` | H100 雲端互動 CLI |
| `advanced/run_h100.py` | HPC 批量執行腳本 |
| `evaluation/` | YOLO Gap 系統性評估框架（基線 / 遮擋 / 歸因實驗）|
| `evaluation/configs/` | 各實驗的 prompt 與查詢配置 |
| `evaluation/scripts/` | 實驗執行腳本與報告產生器 |
| `tests/` | 功能與進階測試腳本 |
| `docs/` | README 用 Demo 圖片 |
